...
# List of packages
packages <- c("devtools", "tidyverse", "janitor", "data.table", "kableExtra", "grid", "cowplot", "boot",
              "gridExtra", "car", "knitr", "plotly", "ggbreak", "patchwork", "tinytex", "marginaleffect",
...
¿Qué drogas se asocian a la hospitalización psiquiátrica dentro del tiempo posterior al alta de un tratamiento por consumo de sustancias, en adultos con TUS atendidos en la red chilena entre 2010-2022?

## Modelo teórico o mecanismo que explicaría la relación entre las variables que se van a analizar e hipótesis

Se parte de un marco de riesgo acumulativo en el que la probabilidad de hospitalización ($p$) surge de la asociación entre:

- Severidad adictiva:  tipo de sustancia como variable independiente principal y número de re-tratamientos, como variable confusora e indicadora de recaídas y cronicidad.

- Vulnerabilidad psiquiátrica previa: número de hospitalizaciones mentales antes del tratamiento por drogas, como variable confusora.

- Características del tratamiento: modalidad (ambulatoria vs residencial).

- Variables sociodemográficos: la principal es el sexo, dado su interaction con el tipo de sustancia, edad (lineal y cuadrática), , estado civil, nivel educacional y región de residencia.

Bajo este esquema se plantea la siguiente hipótesis:

Pacientes con historial de TUS presentan una mayor probabilidad de hospitalización psiquiátrica caundo su sustancia principal de consumo es de depresores del sistema nervioso central.

Este modelo conceptual apoyado por la evidencia internacional sobre patología dual y cursos clínicos severos  guiará la exploración empírica de los datos chilenos [@GomezSanchezLafuente2016]. Evaluar dichas hipótesis permitirá distinguir los pacientes de alto riesgo segun y orientar estrategias integradas que reduzcan la probabilidad de hospitalización, mejoren la continuidad asistencial y optimicen el uso de recursos en salud mental tanto en Chile como en otros contextos comparables.

# Metodología 
## Datos

Este es un estudio de cohorte retrospectiva de pacientes adultos en tratamiento por consumo de sustancias, con datos otorgados por el Servicio Nacional para la Prevención y Rehabilitación del Consumo de Drogas y Alcohol de Chile (SENDA) en convenio con el núcleo milenio de ánalisis de políticas públicas de drogas (nDP). La cohorte se construyó vinculando los registros administrativos de los pacientes (n = 156.771 episodios de tratamiento entre 97,698 personas en las 16 regiones del país) con los datos de egresos hospitalarios a nivel nacional entre 2010 y 2022 (20.652.003 hospitalizaciones).
...
Se incluyeron como **predictores** de la probabilidad de hospitalización ($p$), múltiples variables relacionadas al **consumo y tratamiento rehabilitador de drogas**. Se consideró como la principal variable independiente la *sustancia principal* por la cual se trató al paciente (alcohol, pasta base de cocaína, cocaína, marihuana, depresores del SNC u otras sustancias; tomando Alcohol como categoría de referencia). Luego como variables confusoras (que producen sesgo por omisión), el **número de reingresos** a tratamiento (retratamientos, categorizados en 0, 1, 2, 3 o más reingresos), el *tipo de plan de tratamiento* (ambulatorio vs. residencial) y el **historial clínico** de salud mental del paciente a través del número de hospitalizaciones psiquiátricas previas al tratamiento de drogas (dado la posible autoselección).

Ademas se ajustaron por variables **sociodemográficas** consideradas están la edad (en años) al inicio del tratamiento incorporada también en forma cuadrática ($\text{edad}^2$) para capturar posibles efectos no lineales, el sexo del paciente y su interación por el tipo de sustancia, el **estado civil** (por ejemplo, soltero, casado/conviviente, separado, viudo), el **nivel educacional** alcanzado (educación primaria, secundaria, técnica, universitaria) y la situación de **empleo** (categorías como trabajando, desempleado, estudiando, tareas del hogar, jubilado, etc.). 

Todas las variables categóricas se introdujeron mediante codificación *dummy*, definiendo categorías de referencia apropiadas (p. ej., sexo femenino, tratamiento ambulatorio, etc.)
...
Efecto Parcial en el Promedio (PEA) para evalúar el efecto parcial en el individuo promedio:
...
Efecto Parcial Promedio (APE) que promedia los efectos parciales individuales en toda la muestra:
...
### Análisis de Sesgos de Omisión en Sustancia Principal en Modelos de Regresión Logística Anidados

La Tabla 2 presenta los resultados de los  7 modelos logit estándar. En la tabla 10 Los tests de razón de verosimilitud confirman que cada adición de variables mejora significativamente el ajuste del modelo (todos los p-valores < 0.001), validando la importancia de incluir estas covariables para evitar sesgos de omisión. La pasta base de cocaina experimenta el sesgo de omisión positiva más dramático: su coeficiente se reduce sustancialmente de $\hat{\beta}=0.150$ (Modelo 1) a $\hat{\beta}=0.007$ (Modelo 2) al incluir el plan residencial, manteniéndose cerca de cero hasta el Modelo 6 ($\hat{\beta}=-0.105$) y finalmente estabilizándose en $\hat{\beta}=0.042$ (Modelo 7), sugiriendo que las variables omitidas inflaban artificialmente su efecto inicial. Otras sustancias también sufre sesgo de omisión positiva progresivo, disminuyendo consistentemente de $\hat{\beta}=0.454$ a $\hat{\beta}=0.026$ en el modelo completo, perdiendo significancia estadística. Depresores del SNC muestra un patrón de sesgo de omisión negativa moderado, con su coeficiente reduciéndose gradualmente de $\hat{\beta}=0.569$ a $\hat{\beta}=0.463$, pero manteniendo significancia. La cocainana presenta sesgo de omisión positiva, disminuyendo de $\hat{\beta}=-0.136$ a $\hat{\beta}=-0.154$, mientras que la marihuana muestra estabilidad relativa. El **plan residencial** mantiene un efecto positivo robusto ($\hat{\beta}=0.763$ a $\hat{\beta}=0.279$), y las **hospitalizaciones previas** emergen como un predictor significativo ($\hat{\beta}=0.478$), confirmando que la especificación completa del modelo es crucial para obtener estimaciones insesgadas de los efectos de las sustancias en el outcome estudiado.

### Análisis de coeficientes en el modelo 7

En la tabla 2 se observa el intercepto del Modelo 7 ($\hat{\beta}_0 = -5.395$, p < 0.001, IC 95%: [-5.923, -4.866]) indica que los log-odds de hospitalización psiquiátrica dado el grupo de referencia (mujeres con tratamiento ambulatorio por alcohol, edad promedio, sin hospitalizaciones previas y sin retratamientos) son significativamente negativos, correspondiendo a una probabilidad basal de aproximadamente 0.5%.

En la Tabla 2, Tabla 10 y Figura 5 se observa que el mayor efecto positivo sobre los log-odds de hospitalización psiquiátrica es el consumo de depresores del SNC, tomando alcohol como referencia ($\hat{\beta} = 0.463$, p < 0.001, IC 95%: [0.243, 0.683]). Este efecto se traduce en un incremento de 2.3 puntos porcentuales en la probabilidad de hospitalización cuando se evalúa en el promedio de las covariables (PEA = 0.0234), mientras que el efecto promedio en toda la muestra es de 2.8 puntos porcentuales (APE = 0.0283). El coeficiente se mantiene estable en todos los modelos indicando su robustez. La cocaína mostró una asociación negativa no significativa ($\hat{\beta} = -0.019$, p > 0.05, IC 95%: [-0.154, 0.117]), representando una reducción no significativa de 0.1 puntos porcentuales (PEA = -0.0009, APE = -0.0011). La pasta base no mostró diferencias significativas con respecto al alcohol ($\hat{\beta} = 0.042$, p > 0.05, IC 95%: [-0.068, 0.153]), con un incremento no significativo de 0.2 a 0.3 puntos porcentuales (PEA = 0.0021, APE = 0.0026). La marihuana tampoco mostró efectos significativos ($\hat{\beta} = -0.093$, p > 0.05, IC 95%: [-0.301, 0.116], PEA = -0.0047, APE = -0.0056). Las otras sustancias presentaron un coeficiente positivo pero no significativo ($\hat{\beta} = 0.026$, p > 0.05, IC 95%: [-0.536, 0.587], PEA = 0.0013, APE = 0.0016).

El sexo masculino se asoció con menores log-odds de hospitalización ($\hat{\beta} = -0.105$, p < 0.05, IC 95%: [-0.203, -0.006]). Esto se traduce en una reducción de 0.5 puntos porcentuales en la probabilidad cuando se evalúa en el promedio (PEA = -0.0053) y 0.6 puntos porcentuales como efecto promedio (APE = -0.0064). La edad mostró una relación no lineal significativa: el efecto lineal fue positivo ($\hat{\beta} = 0.089$, p < 0.001, IC 95%: [0.074, 0.105]), indicando que cada año adicional de edad aumenta la probabilidad de hospitalización en 0.45 puntos porcentuales en el promedio (PEA = 0.0045) y 0.55 puntos porcentuales como efecto promedio (APE = 0.0055). El término cuadrático fue negativo ($\hat{\beta} = -0.001$, p < 0.001, IC 95%: [-0.001, -0.001]), sugiriendo que el riesgo de hospitalización aumenta con la edad pero a una tasa decreciente (Tabla 2, Modelo 7; Tabla 10; Figura 5).

Existe una mayor probabilidad de hospitalización psiquiátrica posterior dado un plan residencial versus ambulatorio ($\hat{\beta} = 0.279$, p < 0.001, IC 95%: [0.205, 0.353]). Esto representa un incremento de 1.4 puntos porcentuales en el promedio (PEA = 0.0141) y 1.7 puntos porcentuales como efecto promedio (APE = 0.0170). Comparando los modelos en la Tabla 2, este efecto se redujo considerablemente desde el Modelo 2 ($\hat{\beta} = 0.763$) al incluir hospitalizaciones previas y retratamientos, sugiriendo que parte del efecto inicial se debía a mayor severidad basal en pacientes residenciales (sesgo por omisión negativo).

El número de hospitalizaciones psiquiátricas previas fue el predictor más fuerte ($\hat{\beta} = 0.478$, p < 0.001, IC 95%: [0.455, 0.500]). Cada hospitalización previa aumenta la probabilidad de hospitalización posterior en 2.4 puntos porcentuales en el promedio (PEA = 0.0241) y 2.9 puntos porcentuales como efecto promedio (APE = 0.0292). Este efecto permaneció estable a través de todos los modelos (4-7), indicando robustez ante la inclusión de covariables adicionales (Tabla 2, Tabla 10).

Los retratamientos mostraron una relación dosis-respuesta clara con la hospitalización psiquiátrica según la Tabla 2, Tabla 10 y Figura 5. Comparado con ningún retratamiento, un retratamiento aumentó los log-odds en 1.048 (p < 0.001, IC 95%: [0.986, 1.110]), indicando un incremento de 5.3 puntos porcentuales en el promedio (PEA = 0.0529) y 6.4 puntos porcentuales como efecto promedio (APE = 0.0640) en la probabilidad de hospitalización. Dos retratamientos aumentaron los log-odds en 1.597 (p < 0.001, IC 95%: [1.514, 1.679]), representando 394% más probabilidad de hospitalización o 8.1 puntos porcentuales adicionales en el promedio (PEA = 0.0806) y 9.8 puntos porcentuales como efecto promedio (APE = 0.0975). Tres o más retratamientos mostraron el efecto más pronunciado con un incremento en log-odds de 2.277 (p < 0.001, IC 95%: [2.191, 2.362]), indicando 11.5 puntos porcentuales adicionales en el promedio (PEA = 0.1149) y 13.9 puntos porcentuales como efecto promedio (APE = 0.1390).

Entre las variables educacionales (referencia: otra/no especificada), ninguna categoría mostró efectos significativos en el Modelo 7 según la Tabla 2. La educación primaria ($\hat{\beta} = -0.066$, p > 0.05, IC 95%: [-0.430, 0.297], PEA = -0.0034, APE = -0.0041), secundaria ($\hat{\beta} = -0.097$, p > 0.05, IC 95%: [-0.459, 0.266], PEA = -0.0049, APE = -0.0059), universitaria ($\hat{\beta} = 0.049$, p > 0.05, IC 95%: [-0.322, 0.419], PEA = 0.0025, APE = 0.0030) y técnica superior ($\hat{\beta} = 0.016$, p > 0.05, IC 95%: [-0.352, 0.385], PEA = 0.0008, APE = 0.0010) no difirieron significativamente de la categoría de referencia.

El estado civil casado/conviviente mostró menores log-odds comparado con divorciado/separado ($\hat{\beta} = -0.126$, p < 0.01, IC 95%: [-0.213, -0.039], OR = 0.88), indicando 12% menos probabilidad de hospitalización, traducido en una reducción de 0.6 puntos porcentuales en el promedio (PEA = -0.0064) y 0.8 puntos porcentuales como efecto promedio (APE = -0.0077). Los solteros no mostraron diferencias significativas ($\hat{\beta} = 0.016$, p > 0.05, IC 95%: [-0.070, 0.101], PEA = 0.0008, APE = 0.0010), ni los viudos ($\hat{\beta} = -0.113$, p > 0.05, IC 95%: [-0.345, 0.120], PEA = -0.0057, APE = -0.0069) o aquellos con otro estado civil ($\hat{\beta} = 0.029$, p > 0.05, IC 95%: [-0.508, 0.567], PEA = 0.0015, APE = 0.0018).

En ocupación (referencia: desempleado), trabajar se asoció con menores log-odds ($\hat{\beta} = -0.381$, p < 0.001, IC 95%: [-0.442, -0.319], OR = 0.68), representando 32% menos probabilidad de hospitalización o una reducción de 1.9 puntos porcentuales en el promedio (PEA = -0.0192) y 2.3 puntos porcentuales como efecto promedio (APE = -0.0232). Los jubilados/pensionados mostraron mayores log-odds ($\hat{\beta} = 0.359$, p < 0.001, IC 95%: [0.179, 0.538], OR = 1.43), indicando 43% más probabilidad de hospitalización o un incremento de 1.8 puntos porcentuales en el promedio (PEA = 0.0181) y 2.2 puntos porcentuales como efecto promedio (APE = 0.0219). Otras ocupaciones mostraron una reducción significativa ($\hat{\beta} = -0.119$, p < 0.05, IC 95%: [-0.211, -0.027], OR = 0.89, PEA = -0.0060, APE = -0.0073), mientras que estudiantes ($\hat{\beta} = -0.129$, p > 0.05, IC 95%: [-0.367, 0.110], PEA = -0.0065, APE = -0.0079) y quienes realizan tareas del hogar ($\hat{\beta} = -0.092$, p > 0.05, IC 95%: [-0.201, 0.016], PEA = -0.0047, APE = -0.0056) no mostraron diferencias significativas (Tabla 2, Tabla 10).

La interacción entre sexo masculino y pasta base fue significativa y negativa ($\hat{\beta} = -0.257$, p < 0.001, IC 95%: [-0.385, -0.129]), indicando que el efecto de la pasta base sobre la hospitalización psiquiátrica difiere entre hombres y mujeres. El efecto parcial de esta interacción es de -1.3 puntos porcentuales en el promedio (PEA = -0.0130) y -1.6 puntos porcentuales como efecto promedio (APE = -0.0157). Para hombres tratados por pasta base, el efecto combinado es $0.042 + (-0.257) = -0.215$, sugiriendo menor riesgo que las mujeres tratadas por la misma sustancia. La interacción con cocaína también fue significativa ($\hat{\beta} = -0.165$, p < 0.05, IC 95%: [-0.326, -0.003], PEA = -0.0083, APE = -0.0100), indicando que los hombres tratados por cocaína tienen entre 0.8 y 1 punto porcentual menos de probabilidad de hospitalización comparado con las mujeres. Las interacciones con marihuana ($\hat{\beta} = 0.182$, p > 0.05, IC 95%: [-0.061, 0.425], PEA = 0.0092, APE = 0.0111), depresores ($\hat{\beta} = 0.115$, p > 0.05, IC 95%: [-0.213, 0.442], PEA = 0.0058, APE = 0.0070) y otras sustancias ($\hat{\beta} = 0.465$, p > 0.05, IC 95%: [-0.221, 1.152], PEA = 0.0235, APE = 0.0284) no fueron significativas (Tabla 2, Modelo 7; Tabla 10).

## Modelos Probit

La Tabla 3 presenta los modelos probit, mostrando patrones similares pero con coeficientes escalados según la función de enlace probit. El intercepto del Modelo 7 ($\hat{\beta}_0 = -2.747$) corresponde a $\Phi(-2.747) = 0.003$, donde $\Phi$ es la función de distribución acumulada normal estándar. Los efectos mantuvieron su dirección y significancia: depresores ($\hat{\beta} = 0.240$, p < 0.001, PEA = 0.0268, APE = 0.0292), edad ($\hat{\beta} = 0.040$, p < 0.001, PEA = 0.0045, APE = 0.0049), plan residencial ($\hat{\beta} = 0.155$, p < 0.001, PEA = 0.0173, APE = 0.0188), y hospitalizaciones previas ($\hat{\beta} = 0.242$, p < 0.001, PEA = 0.0270, APE = 0.0294). Las comparaciones entre modelos anidados (Tabla 7) mostraron patrones similares a los modelos logit, con la mayor mejora al incluir hospitalizaciones previas ($\chi^2_{LR} = 2,673.74$, gl = 1, p < 0.001).

## Modelos Logit con Ajuste de Firth

La Tabla 4 presenta los resultados del método de Firth, diseñado para abordar problemas de separación. Los coeficientes fueron virtualmente idénticos a los del logit estándar, sugiriendo ausencia de problemas de separación en estos datos. El Modelo 7 de Firth mostró el mismo patrón de efectos significativos, con mínimas diferencias en las estimaciones puntuales y efectos marginales prácticamente idénticos. Las comparaciones entre modelos (Tabla 8) replicaron los hallazgos de los otros métodos.
...
```{r}
################################################################################
# COMPLETE PARTIAL EFFECTS TABLE WITH BETA COEFFICIENTS AND STANDARD CI
################################################################################

################################################################################
# FUNCTION TO CALCULATE MARGINAL EFFECTS WITH STANDARD CI (NO BOOTSTRAP)
################################################################################

calculate_all_marginal_effects_standard <- function(model, data = BASE_ANALISIS, 
                                                   effect_type = "APE", 
                                                   conf.level = 0.95) {
  ...
  # Separate main effects, interactions, and variables to skip
  main_effects <- var_names[!grepl(":", var_names) & !grepl("region_del_centro", var_names)]
  interaction_terms <- var_names[grepl(":", var_names) & !grepl("region_del_centro", var_names)]
  
  # Combine all variables to process
  all_vars <- c(main_effects, interaction_terms)
  ...
  for(var in all_vars) {
    ...
    if(effect_type == "APE") {
      # APE: Average of individual marginal effects
      me <- mean(scale_factor) * coefs[var]
      
      # Standard error using delta method for APE
      # Var(APE) = Var(mean(scale_factor) * beta) 
      # For simplicity, assuming scale_factor is fixed at its mean
      se <- sqrt(mean(scale_factor)^2 * vcov_matrix[var, var])
      
      # For more accurate SE, we could account for the variance of scale_factor
      # but this requires more complex calculations
      
    } else {
      # PEA: Marginal effect at the average
      X_mean <- colMeans(X)
      linear_pred_mean <- sum(X_mean * coefs)
      
      if(link_type == "logit") {
        p_mean <- 1 / (1 + exp(-linear_pred_mean))
        scale_factor_mean <- p_mean * (1 - p_mean)
      } else {
        scale_factor_mean <- dnorm(linear_pred_mean)
      }
      
      me <- scale_factor_mean * coefs[var]
      
      # Standard error for PEA
      se <- scale_factor_mean * sqrt(vcov_matrix[var, var])
    }
...
all_results_standard <- data.frame()

# Process each model
for (model_name in names(models_list)) {
  model <- models_list[[model_name]]
  
  # Calculate PEA with standard CI
  pea_results <- calculate_all_marginal_effects_standard(
    model, BASE_ANALISIS, "PEA", conf.level = 0.95
  )
  pea_results$Model <- model_name
  pea_results$Type <- "PEA"
  
  # Calculate APE with standard CI
  ape_results <- calculate_all_marginal_effects_standard(
    model, BASE_ANALISIS, "APE", conf.level = 0.95
  )
  ape_results$Model <- model_name
  ape_results$Type <- "APE"
  
  # Combine
  all_results_standard <- rbind(all_results_standard, pea_results, ape_results)
}

################################################################################
# CREATE FORMATTED TABLE WITH STANDARD CI
################################################################################

create_formatted_table_standard_ci <- function(all_results) {
  ...
  effects_table <- all_results %>%
    mutate(
      ...
      # Add significance based on CI
      effect_sig = case_when(
      ...
      Effect_str = sprintf("%.4f%s", Effect, effect_sig),
      SE_str = sprintf("(%.4f)", SE),
      CI_str = sprintf("[%.4f, %.4f]", CI_lower, CI_upper)
    ) %>%
    select(Variable, Model, Type, Beta_str, Beta_SE_str, Effect_str, SE_str, CI_str)
  
  # Create beta coefficients table (only from PEA to avoid duplication)
  ...
  effects_only_table <- effects_table %>%
    select(Variable, Model, Type, Effect_str, SE_str, CI_str) %>%
    pivot_wider(
      names_from = c(Model, Type),
      values_from = c(Effect_str, SE_str, CI_str),
      names_sep = "_"
    )
  ...
  final_table <- beta_table %>%
    ...
    select(
      ...
      Effect_str_Logit_PEA, SE_str_Logit_PEA, CI_str_Logit_PEA,
      Effect_str_Logit_APE, SE_str_Logit_APE, CI_str_Logit_APE,
      Beta_str_Probit, Beta_SE_str_Probit,
      Effect_str_Probit_PEA, SE_str_Probit_PEA, CI_str_Probit_PEA,
      Effect_str_Probit_APE, SE_str_Probit_APE, CI_str_Probit_APE,
      Beta_str_Firth, Beta_SE_str_Firth,
      Effect_str_Firth_PEA, SE_str_Firth_PEA, CI_str_Firth_PEA,
      Effect_str_Firth_APE, SE_str_Firth_APE, CI_str_Firth_APE
    )
  ...
  return(final_table_formatted)
}

# Create the final table
table_standard_ci <- create_formatted_table_standard_ci(all_results_standard)

# Display table
kable(table_standard_ci,
      caption = "Coeficientes Beta, Efecto Parcial en el Promedio (PEA) y Efecto Parcial Promedio (APE) con IC estándar para los modelos 7.",
      format = "latex",
      booktabs = TRUE,
      escape = FALSE,
      align = c('l', rep('c', 9))) %>%
  ...
  footnote(general = c("*** p < 0.001, ** p < 0.01, * p < 0.05 (significance for β coefficients)",
                      "† indicates 95% CI does not contain zero (for marginal effects)",
                      "Standard errors in parentheses; 95% CI in brackets",
                      "CI calculated using normal approximation with delta method standard errors",
                      "PEA = Partial Effect at the Average; APE = Average Partial Effect",
...
#| include: false
################################################################################
# COMPLETE PARTIAL EFFECTS TABLE WITH BETA COEFFICIENTS AND BOOTSTRAP CI (LENTOOOOOOOOOOOOOOO)
################################################################################

################################################################################
# BOOTSTRAP FUNCTIONS FOR APE
################################################################################

# Bootstrap function for a single variable
boot_ape_single <- function(data, indices, model_formula, var_name, link_type = "logit") {
  # Resample data
  d <- data[indices,]
  ...
  tryCatch({
    if(link_type == "logit") {
      ...
      model <- glm(model_formula, data = d, family = binomial(link = "probit"))
    } else if(link_type == "firth") {
      suppressWarnings({
        model <- brglm2::glm(model_formula, data = d, family = binomial(link = "logit"), 
                            method = "brglmFit")
      })
      link_type <- "logit"  # Firth uses logit link
    }
    ...
    # Calculate APE
    ape <- mean(scale_factor) * coefs[var_name]
    
    return(as.numeric(ape))
...
################################################################################
# EXTENDED FUNCTION WITH BOOTSTRAP CI
################################################################################

calculate_all_marginal_effects_with_bootstrap <- function(model, data = BASE_ANALISIS, 
                                                         effect_type = "APE", 
                                                         R = 1000, 
                                                         conf.level = 0.95) {
  ...
  for(var in all_vars) {
    ...
    if(effect_type == "APE") {
      ...
      se <- sqrt(mean(scale_factor)^2 * vcov(model)[var, var])
    } else {
      # PEA: Marginal effect at the average
      X_mean <- colMeans(X)
      linear_pred_mean <- sum(X_mean * coefs)
      ...
      me <- scale_factor_mean * coefs[var]
      se <- scale_factor_mean * sqrt(vcov(model)[var, var])
    }
    ...
    # Bootstrap CI for APE only
    if(effect_type == "APE" && R > 0) {
      
      # Define bootstrap function
      boot_fun <- function(data, indices) {
        boot_ape_single(data, indices, model_formula, var, model_type)
      }
      ...
      if(length(valid_results) > R/2) {  # At least 50% successful bootstraps
        # Calculate confidence intervals
        ci <- boot.ci(boot_results, type = c("perc", "bca"), conf = conf.level)
        
        # Extract CI bounds
        ci_lower <- ifelse(!is.null(ci$bca), ci$bca[4], ci$percent[4])
        ci_upper <- ifelse(!is.null(ci$bca), ci$bca[5], ci$percent[5])
        
        # Bootstrap SE
        se_boot <- sd(valid_results)
      } else {
        ci_lower <- NA
        ci_upper <- NA
        se_boot <- se
      ...
      # For PEA or if no bootstrap requested, use normal approximation
      ci_lower <- me - qnorm((1 + conf.level)/2) * se
      ci_upper <- me + qnorm((1 + conf.level)/2) * se
      se_boot <- se
    }
    ...
    temp_df <- data.frame(
      ...
      Effect = me,
      SE = se,
      SE_boot = ifelse(exists("se_boot"), se_boot, se),
      CI_lower = ci_lower,
      CI_upper = ci_upper,
      Beta = beta,
      Beta_SE = beta_se,
...
# Initialize combined results
all_results_boot <- data.frame()

# Process each model
for (model_name in names(models_list)) {
  model <- models_list[[model_name]]
  
  # Calculate PEA (no bootstrap needed)
  pea_results <- calculate_all_marginal_effects_with_bootstrap(
    model, BASE_ANALISIS, "PEA", R = 0
  )
  pea_results$Model <- model_name
  pea_results$Type <- "PEA"
  
  # Calculate APE with bootstrap CI
  ape_results <- calculate_all_marginal_effects_with_bootstrap(
    model, BASE_ANALISIS, "APE", 
  ...
  # Combine
  all_results_boot <- rbind(all_results_boot, pea_results, ape_results)
}
...
create_formatted_table_with_ci <- function(all_results) {
  ...
  effects_table <- all_results %>%
    mutate(
      ...
      # Add significance based on CI for APE
      effect_sig = case_when(
        Type == "APE" & !is.na(CI_lower) & !is.na(CI_upper) & 
          (CI_lower > 0 | CI_upper < 0) ~ "†",
        TRUE ~ ""
      ),
      Beta_str = sprintf("%.4f%s", Beta, beta_stars),
      Beta_SE_str = sprintf("(%.4f)", Beta_SE),
      Effect_str = sprintf("%.4f%s", Effect, effect_sig),
      SE_str = sprintf("(%.4f)", SE_boot),
      CI_str = ifelse(Type == "APE" & !is.na(CI_lower), 
                     sprintf("[%.4f, %.4f]", CI_lower, CI_upper),
                     "")
    ) %>%
    select(Variable, Model, Type, Beta_str, Beta_SE_str, Effect_str, SE_str, CI_str)
  
  # Create beta coefficients table
  beta_table <- effects_table %>%
    filter(Type == "PEA") %>%
    select(Variable, Model, Beta_str, Beta_SE_str) %>%
    pivot_wider(
  ...
  final_table_formatted <- final_table %>%
    mutate(
      ...
      `Logit PEA` = paste0(Effect_str_Logit_PEA, "\n", SE_str_Logit_PEA),
      `Logit APE` = paste0(Effect_str_Logit_APE, "\n", SE_str_Logit_APE, 
                          ifelse(CI_str_Logit_APE != "", paste0("\n", CI_str_Logit_APE), "")),
      `Probit β` = paste0(Beta_str_Probit, "\n", Beta_SE_str_Probit),
      `Probit PEA` = paste0(Effect_str_Probit_PEA, "\n", SE_str_Probit_PEA),
      `Probit APE` = paste0(Effect_str_Probit_APE, "\n", SE_str_Probit_APE,
                           ifelse(CI_str_Probit_APE != "", paste0("\n", CI_str_Probit_APE), "")),
      `Firth β` = paste0(Beta_str_Firth, "\n", Beta_SE_str_Firth),
      `Firth PEA` = paste0(Effect_str_Firth_PEA, "\n", SE_str_Firth_PEA),
      `Firth APE` = paste0(Effect_str_Firth_APE, "\n", SE_str_Firth_APE,
                          ifelse(CI_str_Firth_APE != "", paste0("\n", CI_str_Firth_APE), ""))
    ) %>%
    select(Variable, 
           `Logit β`, `Logit PEA`, `Logit APE`,
           `Probit β`, `Probit PEA`, `Probit APE`,
...
# Create the final table
table_with_bootstrap <- create_formatted_table_with_ci(all_results_boot)

# Display table
kable(table_with_bootstrap,
      caption = "Coeficientes Beta, Efecto Parcial en el Promedio (PEA) y Efecto Parcial Promedio (APE) con IC Bootstrap para los modelos 7.",
      format = "latex",
      booktabs = TRUE,
      escape = FALSE,
      align = c('l', rep('c', 9))) %>%
  ...
  add_header_above(c(" " = 1, "Logit" = 3, "Probit" = 3, "Firth" = 3)) %>%
  footnote(general = c("*** p < 0.001, ** p < 0.01, * p < 0.05 (significance for β coefficients)",
                      "† indicates 95% bootstrap CI does not contain zero (for APE)",
                      "Standard errors in parentheses; 95% bootstrap CI in brackets for APE",
                      "Bootstrap CI based on 1000 replications using BCa method when available",
                      "PEA = Partial Effect at the Average; APE = Average Partial Effect",
                      "Regional variables excluded from table"))
...
#| fig-height: 14 
#| fig-width: 14
#| fig-cap: "Efecto Parcial en el Promedio (PEA) y Efecto Parcial Promedio (APE) para las interacciones entre sustancia principal y sexo en los modelos 7. La figura muestra los efectos diferenciados por sexo para cada tipo de sustancia comparada con alcohol, calculando la probabilidad de hospitalización psiquiátrica para hombres y mujeres. Los intervalos de confianza se calcularon usando el método delta con aproximación normal."

################################################################################
# FUNCTIONS TO CALCULATE MARGINAL EFFECTS
################################################################################

calculate_interaction_effects <- function(model, data = BASE_ANALISIS, effect_type = "APE") {
  
  # Determine model type and link function
  if(inherits(model, "brglmFit")) {
    link_type <- "logit"
  } else {
    link_type <- family(model)$link
  ...
  # Get design matrix and linear predictor
  X <- model.matrix(model)
  linear_pred <- X %*% coefs
  
  # Calculate scale factor based on link function
  if(link_type == "logit") {
    pred_probs <- 1 / (1 + exp(-linear_pred))
    scale_factor <- pred_probs * (1 - pred_probs)
  } else if(link_type == "probit") {
  ...
  substances <- c("Marijuana", "Other substances", "Cocaine", "Depressants", "Cocaine paste")
  ...
  for(substance in substances) {
    # Coefficient names
    main_effect <- paste0("principal_sub", substance)
    interaction_effect <- paste0("principal_sub", substance, ":sexMale")
    
    # Check if coefficients exist
    if(main_effect %in% names(coefs) && interaction_effect %in% names(coefs)) {
      
      # Effects for females (baseline category)
      if(effect_type == "APE") {
        female_effect <- mean(scale_factor) * coefs[main_effect]
        female_se <- sqrt(mean(scale_factor)^2 * vcov_matrix[main_effect, main_effect])
      } else {
        # PEA: effect at the average
        X_mean <- colMeans(X)
        linear_pred_mean <- sum(X_mean * coefs)
        ...
        female_effect <- scale_factor_mean * coefs[main_effect]
        female_se <- scale_factor_mean * sqrt(vcov_matrix[main_effect, main_effect])
      }
      
      # Effects for males (main effect + interaction)
      male_coef <- coefs[main_effect] + coefs[interaction_effect]
      
      # Variance of linear combination: Var(a + b) = Var(a) + Var(b) + 2*Cov(a,b)
      male_var <- vcov_matrix[main_effect, main_effect] + 
                  vcov_matrix[interaction_effect, interaction_effect] + 
                  2 * vcov_matrix[main_effect, interaction_effect]
      
      if(effect_type == "APE") {
        male_effect <- mean(scale_factor) * male_coef
        male_se <- sqrt(mean(scale_factor)^2 * male_var)
      } else {
        male_effect <- scale_factor_mean * male_coef
        male_se <- scale_factor_mean * sqrt(male_var)
      }
      ...
      results <- rbind(results,
        data.frame(
          Substance = substance,
          Sex = "Female",
          Effect = female_effect,
          SE = female_se,
          CI_lower = female_effect - 1.96 * female_se,
          CI_upper = female_effect + 1.96 * female_se,
          stringsAsFactors = FALSE
        ),
        data.frame(
          Substance = substance,
          ...
          Effect = male_effect,
          SE = male_se,
          CI_lower = male_effect - 1.96 * male_se,
          CI_upper = male_effect + 1.96 * male_se,
          stringsAsFactors = FALSE
        )
      )
    }
...
#' Calculate retreatment effects
calculate_retratamientos_effects <- function(model, data = BASE_ANALISIS, effect_type = "APE") {
...
#' Calculate cumulative effects of previous hospitalizations
calculate_hosp_effects_range <- function(model, data, effect_type = "APE", max_hosp = 10) {
  ...
  # Coefficients
  coefs <- coef(model)
  beta_hosp <- coefs["n_hosp_mental_previas"]
  vcov_matrix <- vcov(model)
  
  # Calculate for each number of hospitalizations
  hosp_range <- 0:max_hosp
  results <- data.frame()
  ...
  if(effect_type == "APE") {
    me <- mean(scale_factor) * beta_hosp
    se <- sqrt(mean(scale_factor)^2 * vcov_matrix["n_hosp_mental_previas", "n_hosp_mental_previas"])
  } else {
    # PEA
    X_mean <- colMeans(X)
    linear_pred_mean <- sum(X_mean * coefs)
    ...
    me <- scale_factor_mean * beta_hosp
    se <- scale_factor_mean * sqrt(vcov_matrix["n_hosp_mental_previas", "n_hosp_mental_previas"])
  }
  
  # Calculate cumulative effects
  for(n_hosp in hosp_range) {
    cumulative_effect <- me * n_hosp
    cumulative_se <- se * sqrt(n_hosp)  # Simplified - assumes independence
    ...
    results <- rbind(results, data.frame(
      ...
      se = cumulative_se,
      lower = cumulative_effect - 1.96 * cumulative_se,
      upper = cumulative_effect + 1.96 * cumulative_se
    ))
  }
...
all_interaction_effects <- data.frame()

for (model_name in names(models_list)) {
  model <- models_list[[model_name]]
  
  # PEA
  pea_results <- calculate_interaction_effects(model, BASE_ANALISIS, "PEA")
  pea_results$Model <- model_name
  pea_results$Effect_Type <- "PEA"
  
  # APE
  ape_results <- calculate_interaction_effects(model, BASE_ANALISIS, "APE")
  ape_results$Model <- model_name
  ape_results$Effect_Type <- "APE"
  
  all_interaction_effects <- rbind(all_interaction_effects, pea_results, ape_results)
}

# Calculate retreatment effects
all_retreat_effects <- data.frame()

for (model_name in names(models_list)) {
  model <- models_list[[model_name]]
  
  # APE
  ape_results <- calculate_retratamientos_effects(model, BASE_ANALISIS, "APE")
  ape_results$Model <- model_name
  ape_results$Effect_Type <- "APE"
  
  # PEA
  pea_results <- calculate_retratamientos_effects(model, BASE_ANALISIS, "PEA")
  pea_results$Model <- model_name
  pea_results$Effect_Type <- "PEA"
  
  all_retreat_effects <- rbind(all_retreat_effects, ape_results, pea_results)
}

# Calculate hospitalization effects
hosp_effects_all <- data.frame()

for (model_name in names(models_list)) {
  model <- models_list[[model_name]]
  
  # APE
  hosp_ape <- calculate_hosp_effects_range(model, BASE_ANALISIS, "APE", max_hosp = 10)
  hosp_ape$Model <- model_name
  hosp_ape$Effect_Type <- "APE"
  
  # PEA
  hosp_pea <- calculate_hosp_effects_range(model, BASE_ANALISIS, "PEA", max_hosp = 10)
  hosp_pea$Model <- model_name
  hosp_pea$Effect_Type <- "PEA"
  
  hosp_effects_all <- rbind(hosp_effects_all, hosp_ape, hosp_pea)
}
...
plot_data_interactions <- all_interaction_effects %>%
  mutate(
    Effect_label = sprintf("%.3f", Effect),
    sig_indicator = ifelse((CI_lower > 0 | CI_upper < 0), "", ""),
    Effect_label_with_sig = paste0(Effect_label, sig_indicator),
    Substance = factor(Substance, 
                      levels = c("Marijuana", "Other substances", 
                                "Cocaine", "Depressants", "Cocaine paste"))
  )

# PLOT 1: Substance-Sex Interactions with improved annotations
p_interactions <- ggplot(plot_data_interactions, 
                        aes(x = Effect, y = Substance, color = Model, shape = Sex)) +
  # Points
  geom_point(size = 4, 
  ...
  # Text annotations for each point
  geom_text(aes(label = Effect_label_with_sig, group = interaction(Model, Sex)), 
            position = position_dodge(width = 0.7),
            hjust = -0.2,
            vjust = -0.5,
            size = 2.8,
            show.legend = FALSE) +
  # Facets
  facet_grid(Sex ~ Effect_Type, scales = "free_x") +
  # Scales and labels
  scale_color_manual(values = cool_colors) +
  scale_shape_manual(values = c("Female" = 16, "Male" = 17)) +
  labs(
    title = "Substance-Sex Interaction Effects (vs. Alcohol)",
  ...
  scale_x_continuous(expand = expansion(mult = c(0.15, 0.5)))

# PLOT 2: Retreatments
plot_data_retreat <- all_retreat_effects %>%
  mutate(
    Level = gsub("retratamientos_cat", "", factor),
    Level = factor(Level, levels = c("1", "2", "3+")),
    ...
    AME_label_with_sig = paste0(AME_label, sig_stars)
  )

p_retreatments <- ggplot(plot_data_retreat, aes(x = AME, y = Level)) +
  ...
  geom_errorbarh(aes(xmin = lower, xmax = upper, color = Model),
                 ...
                 alpha = 0.6,
                 size = 0.8) +
  geom_text(aes(label = AME_label_with_sig, color = Model), 
            position = position_dodge(width = 0.6),
            hjust = -0.2,
            vjust = 0,
            size = 3.5,
...
p_hosp <- ggplot(hosp_effects_all, 
                 aes(x = n_hosp, y = cumulative_effect, color = Model, fill = Model)) +
  ...
  labs(
    title = "Cumulative Effect of Previous Mental Hospitalizations",
    x = "Number of Previous Hospitalizations",
...
title <- ggdraw() +
  ...
  draw_label(
    "PEA (Partial Effect at Average) and APE (Average Partial Effect) with 95% confidence intervals",
    y = 0.9, 
    size = 14, 
    colour = "gray40"
  )
...
# Function to calculate APE for bootstrap
boot_ape_function <- function(data, indices, model_formula, var_name, link_type) {
  d <- data[indices,]
  ...
  tryCatch({
    if(link_type == "firth") {
      ...
      model <- glm(model_formula, data = d, family = binomial(link = link_type))
    }
    
    coefs <- coef(model)
    if(!(var_name %in% names(coefs))) return(NA)
...
calculate_marginal_effects_with_bootstrap <- function(model, data = BASE_ANALISIS, 
                                                     effect_type = "APE", 
                                                     R_boot = 500) {
  ...
  # Calculate scaling factor based on link function
  if(link_type == "logit" || model_type == "firth") {
    pred_probs <- 1 / (1 + exp(-linear_pred))
    scale_factor <- pred_probs * (1 - pred_probs)
  } else if(link_type == "probit") {
    pred_probs <- pnorm(linear_pred)
  ...
  if("n_hosp_mental_previas" %in% names(coefs)) {
    var_name <- "n_hosp_mental_previas"
    
    if(effect_type == "APE") {
      me <- mean(scale_factor) * coefs[var_name]
      se <- sqrt(mean(scale_factor)^2 * vcov(model)[var_name, var_name])
      
      # Bootstrap CI for APE
      if(R_boot > 0) {
        boot_fun <- function(data, indices) {
          boot_ape_function(data, indices, model_formula, var_name, model_type)
        }
        
        boot_results <- boot(data = data, statistic = boot_fun, R = R_boot)
        valid_results <- boot_results$t[!is.na(boot_results$t)]
        
        if(length(valid_results) > R_boot/2) {
          ci <- quantile(valid_results, c(0.025, 0.975))
          lower_boot <- ci[1]
          upper_boot <- ci[2]
        } else {
          lower_boot <- me - 1.96 * se
          upper_boot <- me + 1.96 * se
        }
      } else {
        lower_boot <- me - 1.96 * se
        upper_boot <- me + 1.96 * se
      ...
      me <- scale_factor_mean * coefs[var_name]
      se <- scale_factor_mean * sqrt(vcov(model)[var_name, var_name])
      lower_boot <- me - 1.96 * se
      upper_boot <- me + 1.96 * se
    }
    ...
    results[[var_name]] <- data.frame(
      ...
      lower = me - 1.96 * se,
      upper = me + 1.96 * se,
      lower_boot = lower_boot,
      upper_boot = upper_boot,
      stringsAsFactors = FALSE
    )
  }
  ...
  if(length(substance_vars) > 0) {
    ...
    for(var in substance_vars) {
      if(effect_type == "APE") {
        ...
        if(R_boot > 0) {
          boot_fun <- function(data, indices) {
            boot_ape_function(data, indices, model_formula, var, model_type)
          }
...
################################################################################
# CALCULATE HOSPITALIZATION EFFECTS FOR RANGE
################################################################################
...
calculate_hosp_effects_range <- function(model, data, effect_type = "APE", max_hosp = 10) {
  ...
  # Create results for each number of hospitalizations
  hosp_range <- 0:max_hosp
  results <- data.frame()
  ...
  for(n_hosp in hosp_range) {
    ...
    if(effect_type == "APE") {
      # Marginal effect is constant for linear terms
      me <- mean(scale_factor) * beta_hosp
      se <- sqrt(mean(scale_factor)^2 * vcov(model)["n_hosp_mental_previas", "n_hosp_mental_previas"])
    } else {
      # PEA
      X_mean <- colMeans(X)
      linear_pred_mean <- sum(X_mean * coefs)
      ...
      me <- scale_factor_mean * beta_hosp
      se <- scale_factor_mean * sqrt(vcov(model)["n_hosp_mental_previas", "n_hosp_mental_previas"])
    }
...
# Calculate effects with bootstrap for APE only
for (model_name in names(models_list)) {
  model <- models_list[[model_name]]
  
  # Calculate APE with bootstrap
  ape_results <- calculate_marginal_effects_with_bootstrap(
    model, BASE_ANALISIS, "APE", R_boot = 200
  )
  
  # Calculate PEA without bootstrap
  pea_results <- calculate_marginal_effects_with_bootstrap(
    model, BASE_ANALISIS, "PEA", R_boot = 0
  )
...
plot_data <- all_effects %>%
  mutate(
    Level = case_when(
      Variable == "principal_sub" & grepl("Marijuana", factor) ~ "Marijuana",
      Variable == "principal_sub" & grepl("Other substances", factor) ~ "Other substances",
      Variable == "principal_sub" & grepl("Cocaine paste", factor) ~ "Cocaine paste",
      Variable == "principal_sub" & grepl("Cocaine", factor) & !grepl("paste", factor) ~ "Cocaine",
      Variable == "principal_sub" & grepl("Depressants", factor) ~ "Depressants",
    ...
    AME_label_with_sig = paste0(AME_label, sig_stars),
    # Use bootstrap CI for APE, normal CI for PEA
    lower_ci = ifelse(Effect_Type == "APE", lower_boot, lower),
    upper_ci = ifelse(Effect_Type == "APE", upper_boot, upper)
  )
...
p_substance <- plot_data %>%
  ...
  geom_point(aes(color = Model), 
             ...
             position = position_dodge(width = 0.7),
             alpha = 0.9) +
  geom_errorbarh(aes(xmin = lower_ci, xmax = upper_ci, color = Model),
                 height = 0.2,
                 position = position_dodge(width = 0.7),
                 alpha = 0.6,
                 size = 0.8) +
...
# Plot 3: Previous Hospitalizations (NEW LINE PLOT)
p_hosp <- hosp_effects_all %>%
  ggplot(aes(x = n_hosp, y = cumulative_effect, color = Model, fill = Model)) +
  geom_ribbon(aes(ymin = lower, ymax = upper), alpha = 0.2) +
  geom_line(size = 1.2) +
  geom_point(size = 2, alpha = 0.8) +
  geom_hline(yintercept = 0, linetype = "dashed", color = "red", size = 0.8) +
...
title <- ggdraw() +
  ...
  draw_label(
    "PEA (Partial Effect at Average) with normal CI; APE (Average Partial Effect) with bootstrap CI",
...
El modelo teórico asumió un riesgo acumulativo en que la hospitalización psiquiátrica depende de sustancias depresosaras del sistema nevioso central y que interacciona con sexo. Otras variables importantes que ajsutan el modelo y previene el sesgo por omisión es el historial psiquiátrico: cada hospitalización mental previa aumentó significativamente la probabilidad de otra hospitalización subsecuente. Asimismo, los retratamientos mostraron un marcado efecto dosis-respuesta: pacientes con ≥3 reingresos a tratamiento presentaron una mayor APE de hospitalización que aquellos sin recaídas. El tipo de tratamiento también fue significativo: la atención residencial se asoció con mayor riesgo relativo a la ambulatoria, aunque parte de esta brecha se redujo al controlar la mayor severidad de los casos derivados a residencias. 
...
Este estudio observacional presenta varias limitaciones. Primero, la naturaleza no experimental impide inferir causalidad sólida, dado que pueden existir endogeneidades por variables omitidas; por ejemplo, el efecto aparente del tratamiento residencial y el numeró de retratamientos  podría reflejar en parte la mayor gravedad clínica de sus pacientes, pero que no se midió directamente. Segundo, el uso de registros administrativos conlleva posibles sesgos de medición: las recaídas podrían estar subreportadas (al considerar solo reingresos formales) y solo se registran hospitalizaciones en el sistema de salud, omitiendo episodios no atendidos o diagnósticos no consignados generando una autoselciones de los casos más graves. Tercero, el modelo predictivo mostró limitaciones de ajuste y poder explicativo: la prueba de Hosmer-Lemeshow resultó significativa (falta de calibración) y la capacidad discriminativa fue moderada (AUC ~0.78), lo que sugiere influencia de factores no observados. Cuarto, estos factores no observados genera posibles sesgos de variables omitidas. Entre estas variables están la severidad de la adicción, comorbilidades psiquiátricas específicas, apoyo social y familiar, adherencia al tratamiento, calidad del tratamiento, entre otras. Finalmente, para afianzar las relaciones causales, se recomienda emplear diseños cuasiexperimentales y datos longitudinales de seguimiento. Dichos enfoques permitirían controlar mejor la endogeneidad, examinar la secuencia temporal entre recaídas y hospitalizaciones, y aportar evidencia más sólida para guiar intervenciones que reduzcan el riesgo de hospitalización.
