...
calculate_marginal_effects_standard <- function(model, data = BASE_ANALISIS, 
                                              effect_type = "APE") {
  ...
  if("n_hosp_mental_previas" %in% names(coefs)) {
    ...
    if(effect_type == "APE") {
      ...
      se <- sqrt(mean(scale_factor)^2 * vcov_matrix[var_name, var_name])
    } else {
      # PEA calculation
      X_mean <- colMeans(X)
      linear_pred_mean <- sum(X_mean * coefs)
      ...
      se <- scale_factor_mean * sqrt(vcov_matrix[var_name, var_name])
    }
...
# Calculate effects with standard CI for both APE and PEA
for (model_name in names(models_list)) {
  model <- models_list[[model_name]]
  
  # Calculate APE with standard CI
  ape_results <- calculate_marginal_effects_standard(
    model, BASE_ANALISIS, "APE"
  )
  
  # Calculate PEA with standard CI
  pea_results <- calculate_marginal_effects_standard(
    model, BASE_ANALISIS, "PEA"
  )
...
# Plot 3: Previous Hospitalizations (LINE PLOT)
